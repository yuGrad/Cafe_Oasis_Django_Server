...
def create_user_cafe_profile_df(user_cafe_profile):
    cafe_value_columns_list = ['beverage', 'dessert', 'various_menu', 'special_menu',
                               'large_store', 'background', 'talking', 'concentration', 'trendy_store']
...
def recommend_cafe_base_keyworkd(user_cafe_profile, nearby_cafes_value):
    ...
    nearby_cafes_value_df = nearby_cafes_value_df.drop(
    ...
    top_similarities = nearby_cafes_value_df['similarity'].nlargest(3)
...
def recommend_cafe_base_rating(nearby_cafes_value):
    ...
    cafe_rating_df = pd.DataFrame(
        [cafe_keywords.cafe.cafe_rating for cafe_keywords in nearby_cafes_value], columns=['cafe_rating'])
    nearby_cafes_value_df = pd.concat(
        [nearby_cafes_value_df, cafe_rating_df], axis=1)

    cafe_id_df = nearby_cafes_value_df['cafe']

    # 필요한 값만 사용 + 계산을 위한 타입 변환
    nearby_cafes_value_df = nearby_cafes_value_df[[
        'beverage', 'clean_store', 'service', 'cafe_rating']]
    nearby_cafes_value_df['cafe_rating'] = nearby_cafes_value_df['cafe_rating'].astype(
        float)
